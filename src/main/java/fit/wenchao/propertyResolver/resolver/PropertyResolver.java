...
import fit.wenchao.propertyResolver.utils.*;
...
import static fit.wenchao.propertyResolver.constants.CommonConstants.DEFAULT_CONF_NAME;
import static fit.wenchao.propertyResolver.constants.CommonConstants.PROPERTIES_SUFFIX;
...
public class PropertyResolver implements IPropertyResolver {
    ...
    public Object processListWithNotGenericObjectField(ResettableInputStream resettableInputStream,
        ...
        if (innerClass.isAssignableFrom(String.class)) {
            ...
            result = processNotGenericObjectNestedInListField(
                    resettableInputStream,
                    fullPropertyName,
                    innerClass,
                    field,
    ...
    public Object processStringField(ResettableInputStream resettableInputStream,
                                     String fullPropertyName,
                                     boolean nestedInList) throws IOException {
        ...
        if (nestedInList) {
            ...
            while ((line = propertyIn.readLine()) != null) {
                ...
                boolean matches = Pattern.matches(
                        "^" + RegexUtils.escape(fullPropertyName) + "\\[.*\\]=.*",
                        line);
                if (matches) {
    ...
    public <T> T createConf(ResettableInputStream resettableInputStream,
        ...
        String configEntryPrefix;
        if (!nested) {
            // config.xxx
            configEntryPrefix = getConfigClassPrefix(configClass);
        } else if (!inList) {
            // config.xxx.nested.xxx
            configEntryPrefix = parentPrefix + "." + parentField.getName();
        } else {
            // config.xxx.nested[].xxx
            configEntryPrefix = parentPrefix + "." + parentField.getName() + "[" + listElemId + "]";
        }
        ...
        for (Field field : fields) {
            ...
            String fullPropertyEntryName = configEntryPrefix + "." + fieldName;
